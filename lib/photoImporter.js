...
function cleanBase64(value) {
  ...
  if (!s) return '';
  if (s.startsWith('data:')) return s.split(',').pop() || '';
  return s;
}
...
function photoItemsFromRows(rows) {
  ...
  for (const row of rows) {
    ...
    const imageBase64 = cleanBase64(row.image_base64 || row.image_data || row.image_1920_base64 || row.local_image_base64);
    ...
    if (!imageUrl && !imageBase64 && !recordExternalId) continue;
    out.push({
      rownum: row.__rownum,
      ...
      image_mime: row.image_mime || row.mime || '',
...
async function importPhotoBatch({ odoo, sheet, rows, options = {} }) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (!item.image_url && !item.image_base64) {
        result.status = 'SKIPPED';
        result.message = 'image_url/image_base64 kosong.';
        report.skipped++;
        report.results.push(result);
        continue;
      }
...
async function downloadImage(url, options = {}) {
  ...
  for (let attempt = 0; attempt <= retryAttempts; attempt++) {
    ...
    try {
      ...
      if (contentType && !/^image\//i.test(contentType)) throw new Error(`URL bukan image content-type: ${contentType}`);
