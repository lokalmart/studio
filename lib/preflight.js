  ...
  for (const sheetName of order) {
    ...
    if (model === 'photo_import_queue') {
      for (const row of s.rows) {
        ...
        if (target && !photoTargetIds.has(String(target).trim())) warnings.push({ sheet: s.name, row: row.__rownum, message: `Target foto belum ada di workbook product.template/product.image: ${target}` });
      }
    }
  }
