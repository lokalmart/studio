  ...
  for (const s of sheets) {
    ...
    if (m === 'product.template') {
      for (const row of s.rows) {
        if (row._external_id) productIds.add(String(row._external_id).trim());
      }
    }
    if (m === 'product.image') {
      for (const row of s.rows) {
        if (row._external_id) imageIds.add(String(row._external_id).trim());
      }
    }
  }
  ...
  for (const sheetName of order) {
      ...
      for (const [key, value] of Object.entries(row)) {
        ...
        if (typeof value === 'boolean') {
          warnings.push({
            sheet: s.name,
            row: row.__rownum,
            message: `Nilai boolean terdeteksi pada kolom ${key}. Mode Super Cepat akan mengonversi ke teks TRUE/FALSE untuk mencegah error bool has no lower.`,
          });
        }
      }
    }
    ...
    if (model === 'ir.model.fields') {
      for (const row of s.rows) {
        ...
        if (row.name && !String(row.name).startsWith('x_')) {
          warnings.push({ sheet: s.name, row: row.__rownum, message: 'Custom field Odoo Online sebaiknya prefix x_.' });
        }
        if (row.ttype === 'many2one' && !row.relation) {
          errors.push({ sheet: s.name, row: row.__rownum, message: 'Many2one wajib punya relation.' });
        }
      }
    }
    ...
    if (model === 'photo_import_queue') {
      for (const row of s.rows) {
        ...
        if (!image && !row.image_base64 && !row.local_image_base64) {
          errors.push({ sheet: s.name, row: row.__rownum, message: 'photo_import_queue wajib punya image_url atau image_base64.' });
        }
        if (target && photoTargetIds.size > 0 && !photoTargetIds.has(String(target).trim())) {
          warnings.push({ sheet: s.name, row: row.__rownum, message: `Target foto belum ada di workbook product.template/product.image: ${target}` });
        }
      }
    }
  }
