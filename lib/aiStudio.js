...
const CONTEXT_MODELS = [
  {
    ...
    domain: [],
    fields: [
      'id','name','default_code','barcode','list_price','standard_price','categ_id',
      'public_categ_ids','sale_ok','purchase_ok','active','website_published',
      'type','detailed_type','uom_id','uom_po_id','create_date','write_date'
    ]
  },
  {
...
function existingFields(modelFields, wanted) {
  ...
  const out = [];
  for (const f of wanted) if (modelFields[f] || f === 'id') out.push(f);
...
async function safeSearchRead(odoo, model, domain = [], fields = ['id'], limit = DEFAULT_LIMIT, order = '') {
  ...
  if (!modelFields) return { ok: false, model, error: `Model tidak bisa dibaca atau tidak tersedia: ${model}`, records: [], fields: [] };
...
async function buildAssistantHealth(odoo, payload = {}) {
  ...
  for (const model of ['product.template', 'res.partner', 'project.task', 'website.page', 'ir.model.fields']) {
    modelChecks.push({ model, available: await modelExists(odoo, model), count: await safeSearchCount(odoo, model, []) });
  }
  return {
    generated_at: nowIso(),
    mode: 'read_first',
...
async function buildSchemaScan(odoo, payload = {}) {
  ...
  const fieldDomain = ['|', ['name', 'ilike', 'x_'], ['model', 'in', CORE_MODELS.concat(customModels.map(m => m.model)).filter(Boolean)]];
  const fieldsRes = await safeSearchRead(
    odoo,
    'ir.model.fields',
    fieldDomain,
    ['id','name','field_description','model','ttype','relation','required','readonly','store','state','on_delete','ondelete','modules','copied','index'],
    clampLimit(payload.fieldLimit, 1000),
    'model asc,name asc'
  );

  const riskyFields = (fieldsRes.records || []).filter(f => {
    const ondel = String(f.on_delete || f.ondelete || '').toLowerCase();
    return f.ttype === 'many2one' && f.required === true && (!ondel || ondel === 'set null' || ondel === 'set_null');
  });

  const counts = [];
  for (const model of CORE_MODELS.concat(customModels.map(m => m.model)).filter(Boolean).slice(0, 40)) {
    counts.push({ model, count: await safeSearchCount(odoo, model, []) });
  }

  return {
    generated_at: nowIso(),
    limits: { model_limit: limit, field_limit: clampLimit(payload.fieldLimit, 1000) },
    summary: {
...
async function buildContextExport(odoo, payload = {}) {
  ...
  const externalIds = await safeSearchRead(
    odoo,
    'ir.model.data',
    [['model', 'in', CONTEXT_MODELS.map(x => x.model)]],
    ['id','module','name','model','res_id','noupdate','date_init','date_update'],
    clampLimit(payload.externalIdLimit, 1000),
    'date_update desc'
  );
  ...
  return {
    ...
    version,
    limits: { per_model_limit: limit, external_id_limit: clampLimit(payload.externalIdLimit, 1000) },
    schema,
    datasets,
...
async function buildDataAudit(odoo, payload = {}) {
  ...
  const products = await safeSearchRead(odoo, 'product.template', [], [
  ...
  ], limit, 'write_date desc');
  if (!products.ok) warnings.push({ area: 'products', message: products.error });

  const barcodeMap = new Map();
  for (const p of products.records) {
    if (!p.name) addIssue(issues, 'critical', 'produk', 'product.template', p, 'Produk tanpa nama.', 'Isi name sebelum publish/import ulang.');
    if (!p.default_code) addIssue(issues, 'medium', 'produk', 'product.template', p, 'Internal reference/default_code kosong.', 'Buat kode stabil untuk migration-safe dan scan gudang.');
    if (!p.barcode) addIssue(issues, 'medium', 'produk', 'product.template', p, 'Barcode kosong.', 'Isi barcode jika produk akan discan atau dilacak.');
    if (!p.categ_id) addIssue(issues, 'high', 'produk', 'product.template', p, 'Kategori teknis produk kosong.', 'Pisahkan product.category teknis dari ecommerce category.');
    if (Number(p.list_price || 0) <= 0 && p.sale_ok !== false) addIssue(issues, 'medium', 'produk', 'product.template', p, 'Harga jual 0 atau kosong.', 'Cek apakah produk memang katalog non-jual atau perlu harga.');
  ...
  for (const [barcode, rows] of barcodeMap.entries()) {
    if (rows.length > 1) {
      for (const p of rows) addIssue(issues, 'critical', 'produk', 'product.template', p, `Barcode duplikat: ${barcode}.`, 'Satu barcode hanya boleh untuk satu produk/variant yang benar.');
    }
  }
  ...
  const partners = await safeSearchRead(odoo, 'res.partner', [], [
  ...
  const tasks = await safeSearchRead(odoo, 'project.task', [], [
  ...
  const fields = await safeSearchRead(odoo, 'ir.model.fields', [['name','ilike','x_']], [
    'id','name','field_description','model','ttype','relation','required','on_delete','ondelete','state','store'
  ], limit, 'model asc,name asc');
  if (!fields.ok) warnings.push({ area: 'fields', message: fields.error });
  for (const f of fields.records) {
    const ondel = String(f.on_delete || f.ondelete || '').toLowerCase();
    if (f.ttype === 'many2one' && f.required === true && (!ondel || ondel === 'set null' || ondel === 'set_null')) {
      addIssue(issues, 'critical', 'field', 'ir.model.fields', f, `Custom Many2one required berisiko: ${f.model}.${f.name}.`, 'Gunakan required=False saat create field, lalu validasi data lewat workflow; jika wajib, ondelete harus restrict/cascade.');
    }
  }
  ...
  if (issues.some(i => i.message.includes('Barcode duplikat'))) recommendations.push('Buat patch khusus barcode-safe sebelum import produk/foto berikutnya.');
  if (issues.some(i => i.message.includes('Kategori teknis'))) recommendations.push('Rapikan product.category teknis Lokalmart terpisah dari product.public.category ecommerce.');
  if (issues.some(i => i.area === 'project')) recommendations.push('Konsolidasikan task ke tiga project utama: Ground Zero, Pilot Cirebon, Operasional/Pengembangan Berjalan.');
...
function flattenRecord(record = {}) {
  ...
  for (const [k, v] of Object.entries(record)) {
    if (Array.isArray(v) && v.length === 2 && typeof v[0] !== 'object') {
      ...
      out[k] = v.map(x => Array.isArray(x) ? x.join(':') : String(x)).join(', ');
...
function addSheet(workbook, name, rows) {
  ...
  XLSX.utils.book_append_sheet(workbook, ws, String(name).slice(0, 31));
}
...
async function buildAssistantXlsxExport(odoo, payload = {}) {
  ...
  const context = await buildContextExport(odoo, { includeSchema: false, limit: clampLimit(payload.contextLimit, 300), externalIdLimit: 1000 });
  ...
  addSheet(wb, 'MODEL_COUNTS', scan.counts || []);
  addSheet(wb, 'CUSTOM_MODELS', scan.custom_models || []);
  addSheet(wb, 'CUSTOM_FIELDS', scan.fields || []);
  addSheet(wb, 'RISKY_FIELDS', scan.risky_fields || []);
  addSheet(wb, 'AUDIT_ISSUES', audit.issues || []);
  addSheet(wb, 'AUDIT_RECOMMENDATIONS', (audit.recommendations || []).map((message, index) => ({ index: index + 1, message })));

  const ds = context.datasets || {};
  for (const [model, data] of Object.entries(ds)) {
    const sheetName = model.replace(/\./g, '_').slice(0, 31);
    addSheet(wb, sheetName, data.records || []);
  }
  addSheet(wb, 'EXTERNAL_IDS', context.external_ids || []);
  ...
  return {
    ...
    summary: {
      ...
      context_models: Object.keys(ds).length
    }
  };
}

async function readOnlyRpc(odoo, payload = {}) {
  const allowed = new Set(['search_read', 'fields_get', 'search_count', 'read', 'search', 'name_search']);
  const model = String(payload.model || '').trim();
  const method = String(payload.method || '').trim();
  if (!model) throw new Error('payload.model wajib diisi.');
  if (!allowed.has(method)) throw new Error(`Method tidak diizinkan untuk AI read RPC: ${method}`);
