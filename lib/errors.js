function simplifyError(err) {
  ...
  const rules = [
    [
      /Changing the type of a field is not yet supported/i,
      'Odoo menolak perubahan tipe field. Field custom sudah ada dengan tipe berbeda; lewati field itu atau buat nama field baru.',
    ],
    [
      /External ID.*not found|External ID.*tidak ditemukan|not enough values to unpack/i,
      'Relasi External ID tidak ditemukan atau format relasi tidak valid. Pastikan record induk sudah diimport lebih dulu.',
    ],
    [
      /Access Denied|not allowed|You are not allowed/i,
      'Akses ditolak. Pastikan user Odoo punya hak create/write untuk model ini atau ACL custom model sudah dibuat.',
    ],
  ...
  return {
    message: compact.slice(0, 500) || 'Error tidak diketahui.',
    detail: compact.slice(0, 3000),
  };
}
